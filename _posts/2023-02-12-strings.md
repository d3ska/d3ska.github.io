...
The solution to the O(n<sup>2</sup>) problem above is to use a mutable buffer. In Java, `StringBuilder` maintains an internal character array that grows as needed, so appending a character is amortized O(1):
...
> **Related posts**: [Complexity Analysis](/posts/complexity-analysis/), [Memory](/posts/memory/)
